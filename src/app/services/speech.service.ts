...
export class SpeechService {
  ...
  //
  private readonly MIN_SPEECH_MS       = 300;   // Ignore turns shorter than this
  private readonly PATIENCE_SILENCE_MS = 1400;  // Show "take your time" after this
  private readonly COMMIT_SILENCE_MS   = 2600;  // Send to Whisper after this
  ...
  setApiKey(k: string): void { this.apiKey = k.trim(); }
  ...
  private onVADFrame(data: { rms: number; envelope: number; isSpeech: boolean }): void {
    if (this.turnState === 'inactive' || this.turnState === 'committing') return;
    ...
    if (data.isSpeech) {
      ...
      // Accumulate speech duration for momentum calculation
      if (this.lastSpeechMs > 0) {
        const gap = now - this.lastSpeechMs;
        // Only count as speech time if frames are close together
      ...
      // ── Silence ─────────────────────────────────────────────
      if (!this.hadSpeech) {
        // No speech heard yet — keep waiting, ignore silence
        return;
      }

      // Start silence clock if not already started
      ...
      // Speech momentum: longer answer → more patience
      // Every 10s of actual speech adds 500ms to the commit threshold
      const momentum     = Math.floor(this.totalSpeechMs / 10000) * this.MOMENTUM_PER_10S;
      const commitAt     = this.COMMIT_SILENCE_MS + momentum;
      const patienceAt   = this.PATIENCE_SILENCE_MS;

      if (silenceDuration >= commitAt) {
        // ── DONE — enough silence, commit this turn ───────────
        this.commit();

      } else if (silenceDuration >= patienceAt) {
        // ── Showing patience — medium silence ────────────────
        ...
        // ── Short silence — breath or micro-pause ─────────────
        // Don't change UI at all for short silences
        // This is what was missing before — we were cutting off here
        if (this.turnState === 'speaking') {
          this.turnState = 'silence-short';
          // Keep showing "Recording your answer..." — no UI change
        }
      }
    }
  }
  ...
  private commit(): void {
    if (this.turnState === 'committing' || this.turnState === 'inactive') return;
  ...
  private async processRecording(): Promise<void> {
    ...
    // Guards
    if (!this.hadSpeech || this.chunks.length === 0 || durationMs < this.MIN_SPEECH_MS) {
      this.phase.set('idle');
      this.transcript.set('');
  ...
  private buildAnalysis(text: string, durationMs: number): SpeechAnalysis {
    ...
    const fillerRx  = /\b(um+|uh+|er+|ah+|hmm+|like|you know|i mean|basically|so so|right right)\b/g;
  ...
  async speak(text: string, onEnd?: () => void): Promise<void> {
    ...
    if (!this.apiKey) { this.browserTTS(clean, onEnd); return; }
    ...
    try {
      ...
      this.ttsAudio.onended = () => this.ngZone.run(() => {
      ...
      this.ttsAudio.onerror = () => this.ngZone.run(() => {
        this.isSpeaking.set(false);
        this.phase.set('idle');
        onEnd?.();
      });
  ...
  private browserTTS(text: string, onEnd?: () => void): void {
    ...
    utt.rate = 0.93;
    utt.onstart  = () => this.ngZone.run(() => { this.isSpeaking.set(true);  this.phase.set('ai-speaking'); });
    utt.onend    = () => this.ngZone.run(() => { this.isSpeaking.set(false); this.phase.set('idle'); onEnd?.(); });
    utt.onerror  = () => this.ngZone.run(() => { this.isSpeaking.set(false); this.phase.set('idle'); onEnd?.(); });
  ...
  private startWithScriptProcessor(): void {
    ...
    processor.onaudioprocess = (e) => {
      ...
      envelope += rms > envelope ? 0.40 * (rms - envelope) : 0.012 * (rms - envelope);
